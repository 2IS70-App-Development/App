...
1.  **Packages:** A unified scrollable LazyColumn displaying all of the user's shipments, both sent and received.
    * **Filtering:** At the top of the screen, place a `SingleChoiceSegmentedButtonRow` (Material 3) with two segments: "Sent" and "Received". This toggles which subset of packages is displayed in the list below. The selected filter should be maintained in the ViewModel so it persists during tab switches.
    ...
    * **Navigation:** Tapping a package card navigates to the Package Detail Screen within the Packages navigation graph. This is not a separate bottom nav destination — it is a nested route so the user stays contextually within the Packages tab. The back button returns to the list.

2.  **QR Code Scanner:** A camera-enabled screen using the device hardware to scan external CryptoSeal QR codes. Successful scans parse the data, register the current user into the shipment's chain of custody, and add an entry to the Packages list under "Received".
...
3.  **QR Code Creator:** A form where the user inputs shipment details to generate a unique QR code label. Successful generation adds an entry to the Packages list under "Sent" and initializes the chain of custody with the creator as the first entry.
...
4.  **Activity:** A real-time, chronological feed of scan events across all of the user's shipments. This provides an at-a-glance overview of what is happening without requiring the user to drill into each package individually.
...
5.  **Profile:** A screen displaying the current authorized employee's identity and their contacts directory.
    ...
    * **Settings Access:** A small gear `IconButton` placed in the top app bar of the Profile screen. Tapping it navigates to a separate Settings screen (nested route within the Profile tab) for any future app preferences, keeping the main Profile screen clean and focused.


## Package Detail Screen
This is a full-screen view that opens when a user taps a package card from the Packages tab or an event from the Activity tab. It is a nested navigation destination, not a bottom nav tab.

* **Header Section:** Display the shipment name, current status as a colored `Badge`, recipient or sender information, and the date created.
* **QR Code Section:** A tappable thumbnail of the QR code that expands into a dialog for sharing or printing.
* **Chain of Custody Timeline:** A scrollable vertical timeline implemented as a `LazyColumn` with custom drawn connectors (vertical lines between nodes). Each node displays the handler's name or ID, the timestamp of their scan, and their GPS location if available. The first entry is always the creator, and the last entry is the most recent scan. Highlight the current holder. This is the core accountability feature of CryptoSeal.